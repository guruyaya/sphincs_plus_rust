...
impl<const STEM_HEIGHT:usize> MerkleSigner<STEM_HEIGHT> {
    pub fn new(seed: HashData, context:HashContext) -> Self {
        let num_trees: u64 = (2 as u64).pow((STEM_HEIGHT) as u32);
    ...
    pub fn pair_keys(keys: &Vec<HashData>) -> Vec<HashData> {
    ...
    pub(super) fn get_proof_and_public_key(&self, lowest_layer: Vec<WotsPlus>) -> (HashData, [HashData;STEM_HEIGHT]){
        let mut public_keys: Vec<HashData> = lowest_layer.iter().map(|wots| wots.generate_public_key().public_key).collect();
        let mut merkle_proof = [HASH_DATA_0;STEM_HEIGHT];
        ...
        for i in 0..STEM_HEIGHT {
            ...
            public_keys = Self::pair_keys(&public_keys)
        };
...
mod tests {
    ...
    fn test_pair_keys (){
        ...
        let keys = MerkleSigner::<2>::pair_keys(&to_join);
        ...
        let more_keys = MerkleSigner::<2>::pair_keys(&keys);
        
        assert_eq!(more_keys.len(), 2);
        assert_ne!(more_keys[0], more_keys[1]);
        
        let one_key = MerkleSigner::<2>::pair_keys(&more_keys);
    ...
    fn test_pair_keys_panics() {
            ...
            let _ = MerkleSigner::<2>::pair_keys(&to_join);
    }
    #[test]
